      ...
      <section className="container mx-auto max-w-6xl grid md:grid-cols-3 gap-6 mb-32">
        {TIERS.map((tier, i) => (
          <motion.div
            ...
            className={`p-10 rounded-3xl border ${tier.featured ? "border-[#FF6B6B] bg-[#FF6B6B]/5 relative transform md:-translate-y-4" : "border-white/10 bg-white/[0.02]"}`}
          >
            {tier.featured && (
            ...
            <button
              className={`w-full py-4 rounded-xl text-xs font-bold uppercase tracking-widest transition-colors ${tier.featured ? "bg-[#FF6B6B] text-black hover:bg-white" : "bg-white/10 hover:bg-white hover:text-black"}`}
            >
              Explore More →
            </button>
          </motion.div>
