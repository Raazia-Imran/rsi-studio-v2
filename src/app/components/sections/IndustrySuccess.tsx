...
const INDUSTRIES = [
  {
    id: "01",
    name: "Private Yacht",
    color: "bg-[#2A4B7C]", // Deep Ocean Blue
    tagline: "Simplifying Fleet Connectivity",
    desc: "Advanced on-board digital experience, logistics routing, and seamless satellite connectivity interfaces.",
    duration: "12 Weeks",
    platform: "Web & Tablet App",
  },
  {
    id: "02",
    name: "Aviation",
    color: "bg-[#D95D39]", // Industrial Orange
    tagline: "Elevating Charter Management",
    desc: "Bespoke flight logistics, luxury charter booking systems, and real-time fleet tracking dashboards.",
    duration: "16 Weeks",
    platform: "SaaS Dashboard",
  ...
  },
  {
    id: "04",
    name: "Startups",
    color: "bg-[#7A3E9D]", // Electric Purple
    tagline: "Accelerating Unicorn Trajectories",
    desc: "Rapid MVP development, scalable architectures, and investor-ready presentation systems.",
    duration: "8 Weeks",
    platform: "MVP & Mobile App",
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        ...
        <div className="mb-16">
          <motion.p
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-[#FF6B6B] text-[10px] font-mono uppercase tracking-[0.8em] mb-4"
          >
            Industry Success
          </motion.p>
          <motion.h2
          ...
          >
            SPECIALIZED <span className="text-white/30">INDUSTRY</span>
          </motion.h2>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
            ...
            {INDUSTRIES.map((item, index) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => setActiveIndex(index)}
                  className={`text-left px-8 py-6 rounded-2xl transition-all duration-300 font-display font-bold text-xl md:text-2xl flex items-center justify-between group ${
                    isActive
                      ? `${item.color} text-white shadow-2xl scale-[1.02] border border-white/10`
                      : "bg-white/[0.02] text-white/40 hover:bg-white/[0.06] hover:text-white border border-white/5"
                  }`}
                >
                  {item.name}
                  {isActive && (
          ...
          {/* Right Column: Featured Content Card */}
          <div className="w-full lg:w-2/3 h-[550px] md:h-[600px] relative rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl">
            <AnimatePresence mode="wait">
              <motion.div
                key={activeIndex}
              ...
              >
                {/* Text Content */}
                <div className="flex-1 flex flex-col justify-center relative z-10">
                  <p className="text-white/70 font-mono text-[10px] uppercase tracking-widest mb-4 font-bold bg-black/20 self-start px-3 py-1 rounded-full border border-white/10">
                    {INDUSTRIES[activeIndex].name} Platform
                  </p>
                  <h3 className="text-4xl md:text-5xl font-display font-bold leading-[1.1] mb-6 text-white tracking-tight drop-shadow-lg">
                    {INDUSTRIES[activeIndex].tagline}
                  </h3>
                  <p className="text-white/90 text-sm md:text-base leading-relaxed mb-10 max-w-sm drop-shadow-md font-light">
                    {INDUSTRIES[activeIndex].desc}
                  </p>

                  <div className="flex flex-wrap gap-8 mt-auto bg-black/20 p-6 rounded-2xl border border-white/10 backdrop-blur-sm w-max">
                    <div>
                      <p className="text-white/60 text-[10px] uppercase tracking-widest mb-2 font-mono">
                        Project Duration
                      </p>
                ...
                {/* Device Mockup Visual (Hidden on small mobile for space) */}
                <div className="hidden md:flex flex-1 relative items-end justify-end translate-y-16">
                  <div className="w-[300px] h-[600px] bg-[#050505] rounded-t-[3rem] p-4 shadow-2xl relative border-8 border-white/20">
                    <div className="w-full h-full bg-white/5 rounded-[2rem] overflow-hidden relative border border-white/10 backdrop-blur-xl">
                      {/* Mockup Interface Details */}
                      <div className="absolute top-6 left-6 right-6 h-8 flex justify-between items-center">
                        <div className="w-12 h-3 bg-white/20 rounded-full" />
                        <div className="w-6 h-3 bg-white/20 rounded-full" />
                      </div>
                      <div className="absolute top-20 left-4 right-4 h-32 bg-white/10 rounded-2xl shadow-sm" />
                      <div className="absolute top-56 left-4 right-4 h-16 bg-white/10 rounded-2xl shadow-sm" />
                      <div className="absolute top-76 left-4 right-4 h-16 bg-white/10 rounded-2xl shadow-sm" />

                      {/* Floating Elements in Mockup */}
                      <div className="absolute bottom-20 left-1/2 -translate-x-1/2 w-48 h-12 bg-[#FF6B6B] rounded-full shadow-[0_0_30px_rgba(255,107,107,0.4)] flex items-center justify-center">
